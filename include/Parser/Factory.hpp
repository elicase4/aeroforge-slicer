...
namespace Parser {
	...
	class Factory {
		public:
			...
			static std::unique_ptr<BaseParser> createParser(const std::string& filename, const std::string reader_format = "", const boost::endian::order file_endian = boost::endian::order::big, const float hash_coord_tol = 1e-5f){
				std::string extension = getFileExtension(filename);
				if (boost::iequals(extension, "STL")){
					return std::make_unique<Parser::STLParser>(filename, file_endian, hash_coord_tol, reader_format);
				}
				if (boost::iequals(extension, "STEP")){
					return std::make_unique<Parser::STEPParser>(filename, file_endian, hash_coord_tol);
				}
				if (boost::iequals(extension, "3MF")){
